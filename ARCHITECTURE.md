...
* **Metabolic State:** Tracks biological stamina and terminal exhaustion. Sprinting during an active hunt depletes the stock, ultimately forcing a terminal walking state and bridging an exhaustion scalar (`0.0 - 1.0`) to the Render Engine and DSP audio filters.
* **Geodesic Physics:** Executes strict AABB collision detection by querying the `Environment`'s `SpatialHashGrid`. Using a 2.0-unit radius lookup drops the mathematical friction of movement from O(N) to O(1).
...
* **The Null Anomaly:** A procedural entity constructed from unlit Platonic solids. It pathfinds to the player, exerts acoustic proximity pressure on the audio context, and triggers a "Consumption Event" (a complete reality rebuild and seed mutation) if it breaches the player's bounding box.
