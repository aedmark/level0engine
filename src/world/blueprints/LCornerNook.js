...
export const LCornerNookProfile = (env, ctx) => {
    ...
    return {
        ...
        prob: 0.0842, build: (x, z) => {
            const w1 = buildWall(env.cellSize, 0.5, archBorderMats(env, ctx, random, x, z, env.sharedWallMat, [{index: 5, dx: 0, dz: -1}]), 3.0, 0, 0);
            w1.position.set(x * env.cellSize, 1.5, z * env.cellSize - (env.cellSize / 2) + 0.25);
            addGeometry(w1);
            const w2 = buildWall(0.5, env.cellSize, archBorderMats(env, ctx, random, x, z, env.sharedWallMat, [{index: 1, dx: -1, dz: 0}]), 3.0, 0, 0);
