        ...
        prob: 0.10, build: (x, z) => {
            ...
            if (typeRoll > 0.66) {
                ...
                for (let i = 0; i < burstLength; i++) {
                    ...
                    addGeometry(side2);
                    const roof = buildWall(dirZ ? tunnelW : env.cellSize, dirZ ? env.cellSize : tunnelW, env.sharedWallMat, roofH_block, tunnelH);
                    roof.position.set(segX * env.cellSize, tunnelH + (roofH_block / 2), segZ * env.cellSize);
                    addGeometry(roof);
                    const liningFloor = buildWall(dirZ ? tunnelW : env.cellSize + 0.05, dirZ ? env.cellSize + 0.05 : tunnelW, env.ductMat, liningH);
                    liningFloor.position.set(segX * env.cellSize, liningH / 2, segZ * env.cellSize);
                    addGeometry(liningFloor);
                    const liningCeil = buildWall(dirZ ? tunnelW : env.cellSize + 0.05, dirZ ? env.cellSize + 0.05 : tunnelW, env.ductMat, liningH);
                    liningCeil.position.set(segX * env.cellSize, tunnelH - (liningH / 2), segZ * env.cellSize);
                    addGeometry(liningCeil);
                    const liningSideW = dirZ ? liningH : env.cellSize + 0.05;
                    const liningSideD = dirZ ? env.cellSize + 0.05 : liningH;
