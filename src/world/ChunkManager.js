...
import ChunkLayout from './ChunkLayout.js';
...
export default class ChunkManager {
    constructor(env) {
        this.env = env;
        this.layout = new ChunkLayout(env.chunkSize);
    }
    ...
    async _buildChunkInterior(args) {
        ...
        for (let x = startX; x < startX + env.chunkSize; x++) {
            for (let z = startZ; z < startZ + env.chunkSize; z++) {
                ...
                if (!ctx.isWall) {
                    this.layout.generate(env, ctx, random, startX, startZ, stagingMeshes, isMacroStructure);
                }
                ...
                if (isWall) {
                    ...
                    if (structure && !(this.layout.isAirlockApron(x, z, env) && structure.name === "CRATES OR STAIRWAY")) {
                        structure.build(x, z);
                    } else {
    ...
    _buildEmptyCell(args, state) {
        ...
        const isAirlockApproach = this.layout.isAirlockApron(x, z, env);
