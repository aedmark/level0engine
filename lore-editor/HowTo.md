...
* **Custom Vars & Procedural Math**:
    ...
    * You can also embed actual JavaScript math evaluations inside the string directly, referencing `ctx` variables. *Example:* `ASYNC REPORT #${ctx.seed * ctx.hours % 666}`.
    ...
    The difference between `VARS` and `CORE_VARS`: `VARS` is a computed shortcut derived from other values (including, trivially, an alias for one single existing value like `PEN`); `CORE_VARS` is a brand-new independent random number generated fresh for that playthrough's seed.
