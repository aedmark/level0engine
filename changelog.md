...
- **[GEOMETRY] Impound Fence Height:** The yard's chainlink fencing reused the room-wall height (3.0, floor to ceiling), reading as a solid partition instead of yard fencing you're meant to see over. Every fence dimension — panels, corner posts, gate posts, gate leaf, top rail — now scales off a single 1.8-unit constant instead, restoring the sightlines a chainlink pen is supposed to have.
- **[GEOMETRY] Checkpoint Control Desk:** No longer dresses with its own standalone hazmat suit. Suits belong on the rack now, full stop — the desk area is terminal, screen, and nothing else.
- **[GENERATION] Sector Selection, Now Bag-Based:** Each macro sector was drawn independently at random every time — mathematically fair per draw (1-in-11, no hidden weighting) but with nothing stopping the same handful from clustering together while others went long stretches unseen. Selection now pulls from a shuffled no-repeat bag, reshuffled once exhausted: every sector is guaranteed a turn at least once every 11 macro chunks, not just in the statistical long run.

#### Fixed

- **[GEOMETRY] The Little Yellow Man (Checkpoint Suit Racks):** Suit racks build each hazmat suit as a `Group`, position the group, then bake its child meshes straight into the instancing pipeline — but nothing ever called `updateMatrix`/`updateMatrixWorld` on the group itself, since it's never parented into the live scene graph where that would happen automatically. Every suit in the level baked its children to their bare local offsets instead of the group's actual position, collapsing every rack's suits onto roughly the same spot near world origin — directly under the player's fixed non-warp spawn point. Forcing `suit.updateMatrixWorld(true)` before the bake resolves the group's transform first; suits now render on their own racks instead of piled at spawn. Separately, the rack's own wall offset (1.82, against a wall face at 2.0) left only 0.18u of clearance — less than the suit's own depth — so the rail and suits read as flush against the wall; brought in to 1.5 to match the clearance every sibling prop in the same dress pass already uses.
- **[GEOMETRY] Checkpoint & Annex Man Doors — Pop, Then Stuck Halfway:** One door orientation's leaf geometry is authored flush at rotation 0 with no rest offset needed, but was tagged `closedRot: -PI/2` with nothing ever setting `rotation.y` to match at creation — Three.js meshes default to 0, so the door displayed closed correctly by coincidence while the animation system privately tracked "closed" as -90°. First interaction snapped the visual from its true 0 toward that phantom baseline instead of swinging smoothly, and closing settled at the wrong -90° instead of true flush — a door leaf sitting perpendicular in its own frame, reported as "pops open, then reverts to stuck halfway." The companion orientation happened to use `closedRot: 0`, which matched its default rest rotation by coincidence, so it never showed the problem. Both the checkpoint man door and the identical annex-door builder elsewhere in the level shared this exact defect (same cached geometry, same wrong constant) — fixed in both, with an explicit `useXApproach` flag added so the swing-direction check still picks the correct side now that `closedRot` alone can't signal orientation.
- **[GEOMETRY] Impound Vehicle Wheels:** `halfL`/`halfW` were re-derived per orientation with the same axis swap the wheel-placement loop already applied a few lines down — the two swaps cancelled out for lengthwise cars (correct by coincidence) and doubled up for sideways ones, placing wheels at the car's half-*length* on the width axis instead of its half-width. Every sideways-oriented car had its wheels floating nearly a full car-width off to the side of the body. `halfL`/`halfW` now stay fixed to the car's own length/width; the orientation swap happens exactly once, where it belongs.
- **[GEOMETRY] Impound Vehicle Hood Tags:** The impound case-file tag pinned to a car's own pivot point, which sits dead center under the cabin and windshield (cabin spans y 0.84–1.40, overlapping the tag's old y 1.02). The tag rendered embedded inside the car's own cabin geometry — invisible, but still hit by the interact raycast, which doesn't check occlusion, so pressing E near a car could return a document with nothing visible to have triggered it. The tag now sits at the same longitudinal inset already used for the wheels (clear of the cabin's footprint by construction) and rotates with the body, landing cleanly on the exposed hood or trunk.
- **[GEOMETRY] Impound Vehicles vs. Fences:** A 3.4-long car has only ~0.3u of built-in clearance to its 4.0-unit cell edge even before any placement jitter — the old +/-0.2 position jitter and +/-0.15 rotation could eat straight through that margin into a fence run on an adjacent wall cell. Both tightened so the worst case comfortably stays inside the cell.
- **[GEOMETRY] Corridor Wall Vents Buried in Their Own Walls:** Wall-mounted vents pick one of 4 faces on their host wall cube at random, with no awareness of what's actually next door. Every wall cube already overlaps its neighbor's cell slightly to hide seams, so whenever the chosen face happened to back onto another solid wall cell, that neighbor's own geometry reached back further than the vent's 0.06u protrusion ever cleared — the vent rendered fully embedded in the neighboring wallpaper, present but permanently unseen. Face selection now re-runs the same fractal pass that decides wall-vs-open for the maze itself (a pure function of cell coordinates, no PRNG calls, so it can safely check neighbors without desyncing the chunk's random stream) and biases toward faces that are actually predicted open.
- **[GENERATION] Airlocks Opening Onto Solid Walls:** Seven sector types (Archive, Server, Maintenance, Impound, Atrium, Chasm, Clinic) share one maze generator: a proper connected spanning-tree carve from the center outward, followed by unconditionally forcing the two cells nearest each of the sector's 4 gates open — regardless of whether the carve's random walk had actually reached that far. When it hadn't, a gate opened onto an isolated one-or-two-cell pocket, walled off from the rest of the sector on every other side: open the airlock, take a step, meet drywall. The generator now forces the full row and column through the maze's center open instead of just the gate-adjacent cells, guaranteeing an unbroken line from every gate to the center — the same guarantee Checkpoint's hand-built corridor already had.

## [v0.4.13] - 2026-07-21

_The Man Door & Impound Yard Update_

#### Added

- **[UI] Coordinate Teleportation:** Added a Teleport (X / Z) input feature to the debug control panel, allowing instant relocation to specific coordinates with automatic chunk generation to prevent falling into the void.

- **[GEOMETRY] The Man Door & Checkpoint Side Rooms:** The Checkpoint's arms were solid fill with nothing behind them. A share of the fill cells that flank a corridor arm now hollow into pockets entered through a new door asset — the man door: a gray steel fire door built from the Research Annex's steel-door and brushed-steel-frame materials, set in checkpoint concrete under a hazard lintel so it reads from the hall, and wired into the same hinged-door system as every other interactive door (swings on approach, the Anomaly can shoulder through). Selection is a pure hash of the cell, so the rest of the chunk generates identically to before, and a run-exclusion rule guarantees every pocket's three non-door faces back onto solid fill — proven across 400 chunks: zero adjacent rooms, zero invalid placements, deterministic. Behind the door sits one of four seeded outcomes: a steel shelving unit stocked with cartons, a pallet-and-drum overflow pile, a genuine supply cache (battery + almond water — a real reward for the detour), or the rare one, an unlit room with a single chair facing the back wall in the dark. Tuned to ~3–4 rooms per checkpoint. One deliberate tension: the rooms are dead-ends with a door the entity can open, so ducking into one mid-chase is a gamble, not a safe room.

- **[GEOMETRY] The Impound Yard Fills Up:** The Impound was chainlink pens and cardboard boxes — a lockup with nothing locked up. Open pen cells now spawn actual impounded property via a new procedural builder: cars (a body, cabin, tinted windshield and four wheels — sometimes up on cinder blocks with a wheel pair missing, in proper impound fashion), skid-mounted machinery (a compressor with a horizontal tank, exhaust stack, and a hazard-yellow control panel), and leaning stacks of three to six tires. Everything is assembled from boxes and cylinders on the existing shared materials — no imported assets — and placed through `addFurniture`, so each item gets one collision box and is skipped rather than clipped if it would foul a fence. Cars carry an impound tag on the hood (~60%), which is a real case-file document, so adding vehicles feeds the paper-trail mechanic instead of bypassing it. The old box/table/chair clutter still fires as a fallback so pens stay varied.

- **[LIGHTING] Chasm Pillar Uplighting:** The Chasm's tower pillars rose out of the black fog lit by nothing. Each now gets a floor-set flood at its base — a hot red-orange fixture aimed at the nearest walkway — so the lower shaft glares out of the fog and fades to dark as it climbs. The aim logic scans the pillar's four neighbours and points the lit face at whichever one is actual catwalk, so it greets the player rather than pointing into the void.

#### Changed

- **[TEXTURES] Server Room Floor Design:** Redesigned the procedural vent generation to use a clean grid of 12x12 square holes with simulated depth instead of horizontal slats. This significantly reduces texture aliasing and Moiré patterns.

- **[WEBGL] Anti-Aliasing & Filtering:** Switched the WebGLRenderTarget and procedural carpet texture from `NearestFilter` to `LinearFilter` and `LinearMipmapLinearFilter`. Enabled 4x MSAA on the render target to eliminate shimmering edges and Moiré artifacts while preserving the ability to emulate retro graphics via internal resolution scaling.

- **[GEOMETRY] The Checkpoint Core (formerly the weird yellow partition):** The arms' crossing at cell (7,7) held a freestanding squeeze-partition — a slab of the overworld's yellow Backrooms wallpaper floating in the middle of a steel quarantine post, trimmed in dark wood-grain frames that read as unexplained black rectangles stuck to nothing. It's gone, and with it those frames. In its place: a giant floor-to-ceiling column of monitors and cables — a 1.3u equipment core (collision, entity blocker) clad on all four faces in a grid of screens (some live and glowing, most dead), topped with a metal cap and a cable trunk climbing a corner into the ceiling, six loose cables draping down, and cable coils pooled at the base. It's still the bottleneck — ~1.35u of walkable margin on every side means you route around it instead of squeezing through a gap. Deterministic from the chunk hash; no PRNG draws, and verified to build a single entity-blocker core with 76 decorative pieces and no degenerate geometry.

- **[LIGHTING] Chasm Red Lights, Grounded:** The Chasm's red catwalk lamps hung at y≈3.8 on ten-unit ceiling wires at `baseIntensity` 0.8 — up out of the fog they were meant to cut through, which in this engine's model (a shared PointLight pool where cast brightness is `baseIntensity × distance-fade × slot-scalar` and colour comes from the fixture's emissive) meant they lit the ceiling and little else. They now sit at y≈0.55 on short deck stanchions — down in the fog layer — at `baseIntensity` 2.4 (3× brighter), hugging a walkway edge overlooking the drop instead of dead-center so they line the catwalks and rake red out into the black. The pointless ceiling wire is gone, the spawn rate nudged from 15% to 22%, and fewer of them are faulty so more stay lit and actually pierce rather than strobing in and out. (The little lamp mesh's own emissive glow is still clamped by the LumenGrid each frame; the added brightness is all cast light, which is what cuts fog.)

#### Fixed

- **[GENERATION] The Empty Chasm:** Pillars were a 5%-per-void-cell coin flip gated to the central band, which made a whole Chasm chunk rolling zero pillars roughly a one-in-eight event — reported, correctly, as "the first chasm I found had zero pillars." Each Chasm chunk now seeds 3–5 pillars up front onto cells it has confirmed are void (so it can never come up empty), with the old random scatter still stacking more on top and loosened from 5% to 10%. The seeding runs on its own local RNG so the chunk's main generation stream is untouched, and it's deterministic — same seed, same chasm. Verified across 1,200 chunks at four maze densities: zero empty chasms, every seeded pillar lands on an actual void cell, layouts stable.

- **[GEOMETRY] Checkpoint Man Doors vs. The Dress:** A man door opens into the corridor arm in front of it, but the arm's quarantine dress — the hall-spanning decon curtain, side-hugging crate stacks, suit racks, drums, AV carts — was placed blind and fouled the doorway. The dress now checks, before it places anything, whether a man door opens onto the cell: the curtain (which spans the whole hall) is skipped near any door, and side props shift to the clear wall or drop entirely if both walls carry a door. Cells with no adjacent door draw the exact same PRNG sequence as before, so nothing else in the checkpoint shifts. Verified across 600 chunks — 1,938 arm cells facing a door — a prop lands on a door side zero times.

- **[GEOMETRY] The Clinic Ceiling Collapse (formerly a giant white block):** The clinic's crouch-under obstacle was a single full-cell `ceilMat` cube hung at belly height — flat, white, referent-free, and because it filled the cell edge-to-edge in *both* directions it clipped straight through the privacy screens and gurneys in neighbouring corridor cells (which jitter past the shared boundary). Rebuilt on both counts. It now spans the full corridor *width* (no sidestep — you still have to crouch) but only 2.6u along the direction of travel, leaving clearance at the cell ends where the neighbouring screens and cots actually sit; the crouch mechanic is untouched, belly still at 1.22 with the mass running up to the ceiling. And it reads as a cave-in: a concrete structural mass carrying the collision, a ceiling-tile skin on the underside so it still reads as roof, a tilted broken slab sagging at the torn edge, exposed rebar raking out of the break, a severed duct end poking from the cavity, and fallen tiles with a leaning strut on the floor. Every hanging piece is pinned at or above the belly — verified across 40,000 random-rotation samples, the lowest debris point is 1.230, just above the 1.22 crawl line — so nothing dangles into the space your head passes through.

## [v0.4.12] - 2026-07-21

_The Ghost Protocol & Exhumed Records Update_

#### Added

- **[DEBUG] The Ghost Protocol:** God mode (`G`) grew from a stat top-up into a real debug rig. **Invincibility:** the Anomaly's kill check now respects god mode — it still hunts, corners, and stands inside you, it just can't finish the bite — and falling below the void plane places you back at y=3 with zeroed velocity instead of mutating the seed, which was quietly the worse debugging tax: one slip in the Chasm used to cost the entire world under inspection. **Ghost flight:** collision negated, `Space` ascends, `C` (held) descends, WASD at a fixed 110 (200 sprinting) regardless of exhaustion. The subtle traps are handled — flying through a wall no longer trips the auto-crouch/auto-squeeze detection (which would have folded the ghost into a 15-speed crawl mid-wall), the crouch toggle is suppressed so descending doesn't land you crouched, and the lean offset zeroes while cinematics are skipped so the camera can't drift a few millimeters sideways per frame. **Debug HUD:** backtick toggles a corner readout refreshing at 4Hz — seed string and hash, active sector, chunk coords, position, FPS, draw calls and triangles straight from `renderer.info` (the live scoreboard for the shadow scheduler and geometry cache), anomaly distance with grace timer and chase state, unclaimed-POI count and hops remaining, fixture and chunk counts, objectives, Coherence.

#### Changed

- **[GEOMETRY] The Door to Nowhere (formerly: a giant blue rectangle):** The oldest signal anomaly — a violet emissive slab — predated the detour catalog's standards and read as unfinished geometry, because it effectively was. First pass dressed it as a leaning monolith with a glowing rim, buckled tiles, and matching floor/ceiling stains; better, but still a shape without a referent, and this facility's horror vocabulary isn't shapes — it's domestic things in wrong places. Final form leans on the fiction's own obsession ("It doesn't use the doors. It doesn't need the doors. Why did we build doors?"): a standard wood-grain door in its jamb, standing free in the middle of the room, attached to no wall, sealed. The door is deliberately undersized in its frame, and a violet glow plate — thinner than the door, sandwiched inside its depth, but wider and taller — burns through every gap: 6cm up each side, 7cm under the header, and 3cm of light spilling out under a closed door. Geometry-verified: no coplanar faces, seams tucked at the jamb line, center occluded from both sides. It has no interaction handler, which the Research Annex already taught players to dread: the only way to know is to try. It won't answer.

#### Fixed

- **[NARRATIVE] The Buried Finding of Fact:** Macro zones lay a foundation plane at y=0.02. Floor documents spawned at y=0.015 — half a centimeter *under the floor*. Invisible, but their interaction boxes stayed live in the spatial grid, which produced the reported haunting: aiming at the records room's almond water served the sealed Finding of Fact from a paper that could not be seen, while the room appeared to contain no note at all. Three spawn sites were buried — the records room's sealed Finding, the Annex's loose office-floor documents, and the Archive stacks' scattered personnel sheets — meaning every floor paper in every foundation-bearing zone has been invisible since foundations were introduced, and the climax document the entire Inquest hinges on was physically unobtainable except by accidentally fishing its hitbox through the tile. All three sites raised to y=0.035 with a warning comment on the constant; the overworld's dangle-vignette sheets stay at 0.015 correctly (no foundation out there), and the Impound's property tags always rode safely on top of their carton stacks. Every honest playtest of the Inquest before this fix was a forced one-in-three guess. The records room now pays out what the code always promised.
...
- **[ATMOSPHERE] Backward Exit Fog Snap:** Sector detection is a point-in-rectangle test, and the blast door sits flush on that rectangle's edge. Walking in is seamless because the door's sector-force bridges the threshold — but its `approaching` check requires moving *toward* the door, so backing out through an open door failed every condition at once: outside the rect, moving away, force dead. The fog snapped to the overworld's thin haze the frame you crossed the line, with the door still open and the zone's whole interior on display, unfiltered. Considered and rejected: rebuilding the v0.4.9 dual-shell airlock (removed deliberately — the corridor bled generic architecture into every zone's identity; not trading a sensor bug for an identity bug). Actual fix: a Schmitt trigger on sector detection. A zone is *entered* at its tight bounds but only *released* 10 units past them — by which point the slider has sealed the sightline. The sticky is set exclusively by the rect test, never by the door force, so skirting past a zone's open door from outside still can't glue its atmosphere to you along the wall. Verified against the full walk path: enter, back out, hold through the threshold, release beyond the margin, no ghost atmosphere on re-approach.
