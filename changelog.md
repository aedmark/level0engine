...
- **[WORLD] Wall Breach Rubble Is Load-Bearing Now:** The rubble variant of `WallBreach.js` built its two leaning slabs and crooked lintel as raw `BoxGeometry` meshes grouped under a staging node — which meant they skipped `buildWall`'s UV rescale (wallpaper tiled at the wrong frequency) and never picked up the `baseboardFootprint` tag that gets a wall a floor trim, since `addGeometry`'s baseboard pass reads world position and rotation off the mesh itself, not a parent group transform. The slabs are now placed directly via `buildWall` with their world transform computed by hand to match, and both are tagged `isEntityBlocker` (previously nothing in the breach blocked the Anomaly's pathing at all). The flat crooked lintel is replaced with `buildSaggingHeader` — a new `StructureKit` primitive that dips a header down to a forced-crouch clearance at the midpoint along a solved circular arc, flush with the pillars at both ends — so a breach the player walks *through* looks like it's caving in along their path, not tilted to one side of the doorway.
